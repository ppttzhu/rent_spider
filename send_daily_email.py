...
def send_daily_email():
    ...
    now = datetime.now()
    today_rooms = [
        room for room in rooms if now - timedelta(hours=24) <= room["fetch_date"]
    ]
    ...
    for location in location_set:
        today_location_rooms = [
            room for room in today_rooms if room["location"] == location
        ]
        top_rooms = get_top_rooms(rooms, location)
        send_notification_email_summary(
            today_location_rooms, [], [], top_rooms, location
        )
