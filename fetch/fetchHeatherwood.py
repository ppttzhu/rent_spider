...
class FetchHeatherwood(Fetch):
    def fetch_web(self):
        ...
        if self.check_availability():
            ...
            return
        attr_filter = 'contains(@class, "btn-viewDetails")'
        apply_buttons = self.wait_until_xpath(
            f"//a[{attr_filter}]|//button[{attr_filter}]"
        )
        for index in range(len(apply_buttons)):
            self.fetch_room_info(index)

    def fetch_room_info(self, index):
        attr_filter = f'@data-selenium-id="FPButton_{index}"'
        apply_button = self.wait_until_xpath(
            f"//a[{attr_filter}]|//button[{attr_filter}]"
        )[0]
        if "contact" in apply_button.text.lower():
            return
        self.move_to_element(apply_button)
        apply_button.click()
        ...
        for room_idx in range(len(room_list)):
            ...
            self.web_wait.until(
                EC.presence_of_element_located(
                    (
                        By.XPATH,
                        f'//input[@data-selenium-id="btnUnitSelect{str(room_idx + 1)}"]',
                    )
                )
            ).click()
            self.driver.switch_to.window(self.driver.window_handles[0])
            ...
            room_price = self.wait_until_xpath(
                '//div[@id="divPricingInfo"]/div/div/label'
            )[-1].text
            self.add_room_info(
